...
void bm2d_seq(long n, long m, REAL *u, int radius, REAL *filter, int num_its);
void bm2d_omp(long n, long m, REAL *u, int radius, REAL *coeff, int num_its);
...
//	bm2d_seq(n, m, u, radius, coeff, num_its);
	...
	int i;
	int num_runs = 1;
//	for (i=0;i<num_runs;i++) bm2d_omp(n, m, u_omp, radius, coeff, num_its);
	omp_elapsed = (read_timer_ms() - omp_elapsed)/num_runs;

	omp_init_devices();
//	printf("OMP mdev execution\n");
//	REAL mdev_elapsed = 0.0;
//	mdev_elapsed = bm2d_omp_mdev(n, m, u_omp_mdev, radius, coeff, num_its);
	...
	mdev_iterate_elapsed = bm2d_omp_mdev_iterate(num_targets, targets, n, m, u_omp_mdev_iterate, radius, coeff, num_its);
...
void bm2d_seq_normal(long n, long m, REAL *u, int radius, REAL *coeff, int num_its) {
	long it; /* iteration */
...
void bm2d_seq(long n, long m, REAL *u, int radius, REAL *coeff, int num_its) {
	long it; /* iteration */
	long u_dimX = n + 2 * radius;
	long u_dimY = m + 2 * radius;
	int coeff_dimX = 2*radius+1;
...
#if 0
void bm2d_omp_mdev(long n, long m, REAL *u, int radius, REAL *coeff, int num_its) {
	long it; /* iteration */
	long u_dimX = n + 2 * radius;
	long u_dimY = m + 2 * radius;
	int coeff_dimX = 2 * radius + 1;
