...
void jacobi_nvgpu_cuda_wrapper2(omp_offloading_t *off, long n,long m,REAL *u,REAL *uold,long uold_n, long uold_m, int uold_0_offset, int uold_1_offset) {
int threads_per_team = omp_get_optimal_threads_per_team(off->dev);
		...
		jacobi_nvgpu_cuda_kernel2<<<teams_per_league, threads_per_team, 0,off->stream->systream.cudaStream>>>(n, m,(REAL*)u,(REAL*)uold, uold_m,uold_0_offset, uold_1_offset);
}


void jacobi_nvgpu_cuda_wrapper1(omp_offloading_t *off, long n,long m,REAL omega,REAL ax,REAL ay,REAL b,REAL *u,REAL *f, \
 REAL *uold, long uold_m, int uold_0_offset, int uold_1_offset, int i_start, int j_start, REAL *error) {
		int threads_per_team = omp_get_optimal_threads_per_team(off->dev);
		int teams_per_league = omp_get_optimal_teams_per_league(off->dev, threads_per_team, n*m);

		/* for reduction operation */
		REAL * _dev_per_block_error = (REAL*)omp_map_malloc_dev(off->dev, NULL, teams_per_league * sizeof(REAL));
		...
		jacobi_nvgpu_cuda_kernel1<<<teams_per_league, threads_per_team,(threads_per_team * sizeof(REAL)),
				...
				omega, ax, ay, b, (REAL*)u, (REAL*)f, (REAL*)uold,uold_m, uold_0_offset, uold_1_offset, i_start, j_start, _dev_per_block_error);
		...
		*error = xomp_beyond_block_reduction_float(_host_per_block_error, teams_per_league, XOMP_REDUCTION_PLUS);
		//cudaStreamAddCallback(__dev_stream__[__i__].systream.cudaStream, xomp_beyond_block_reduction_float_stream_callback, args, 0);
		omp_map_free_dev(off->dev, _dev_per_block_error, 0);
}
