...
int main(int argc, char **argv)
{
    ...
    sumGPU = 0;
    sumCPU = 0;
#pragma omp parallel shared(plan,sumGPU, sumCPU,GPU_N) private(i,j,gpuBase)
{
...
#pragma omp master
{
    printf("Computing with OpenMP and %d GPUs...\n", GPU_N);
    StartTimer();
} 
...
//#pragma omp parallel for reduction(+:sumGPU)
    for (i = 0; i < GPU_N; i++)
    {
        sumGPU += h_SumGPU[i];
    }
#pragma omp master
{
    printf("  GPU with OpenMP Processing time: %f (ms)\n\n", GetTimer());
    ...
    printf("Comparing GPU-OMP and Host CPU results...\n");
