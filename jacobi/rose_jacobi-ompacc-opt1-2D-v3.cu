...
Change the block size to match what HMPP has
Change the loop scheduling method to be round-robin, instead of even division.
*/
...
#endif
#if 1 // 2-D mapping, round robin scheduling
__global__ void OUT__1__10550__(int n,int m,float omega,float ax,float ay,float b,float *_dev_per_block_error,float *_dev_u,float *_dev_f,float *_dev_uold)
{
  float _p_error;
  _p_error = 0;
  ...
  int first_gang_iter_1 = blockDim.x*blockIdx.x +1 ;
  int last_gang_iter_1 = ((first_gang_iter_1 + blockDim.x ) < (n-1) ? (first_gang_iter_1 + blockDim.x) : n-1);

  int first_gang_iter_2 = blockDim.y*blockIdx.y +1 ;
  int last_gang_iter_2 = ((first_gang_iter_2 + blockDim.y ) < (m-1) ? (first_gang_iter_2 + blockDim.y) : m-1);


  int _dev_i; // = blockDim.x * blockIdx.x + threadIdx.x;
  int _p_j; // = blockDim.y * blockIdx.y;

 // if (_dev_i >= 1 && _dev_i <= (n - 1) - 1) {
  for (_dev_i = first_gang_iter_1 + threadIdx.x;  _dev_i <last_gang_iter_1 ; _dev_i += blockDim.x) {
   for (_p_j = first_gang_iter_2 + threadIdx.y;  _p_j < last_gang_iter_2; _p_j+= blockDim.y) {
...
#if 1  // 2-D mapping using round-robin scheduling: each thread grabs an iteration, until nothing left
__global__ void OUT__2__10550__(int n,int m,float *_dev_u,float *_dev_uold)
{
  int first_gang_iter_1 = blockDim.x*blockIdx.x; 
  int last_gang_iter_1 = ((first_gang_iter_1 + blockDim.x ) < n ? (first_gang_iter_1 + blockDim.x) : n);

  int first_gang_iter_2 = blockDim.y*blockIdx.y;
  int last_gang_iter_2 = ((first_gang_iter_2 + blockDim.y ) < m ? (first_gang_iter_2 + blockDim.y) : m);
